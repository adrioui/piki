...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @piki/ai <command> [provider]
...
Examples:
  npx @piki/ai login              # interactive provider selection
  npx @piki/ai login anthropic    # login to specific provider
  npx @piki/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @piki/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @piki/ai --help' for usage`);
	process.exit(1);
}
