...
- Removed dead `core/harness/adapter.ts` (`harnessToolToAgentTool`, zero call sites) and `core/harness/toolkit.ts` (`Toolkit`/`createToolkit`/`mergeToolkits`, zero call sites). Re-exported `expandScratchpadPath` from the harness barrel to preserve the public surface. ([wave2-plan-orphaned](scratchpad/reports/wave2-plan-orphaned.md))
- Removed the dead in-memory KV store (`storage/src/memory-kv.ts`) and its duplicate `coding-agent/src/effect/memory-storage.ts`; both were only re-exported and tested with no runtime call sites. Deleted the `MemoryStorage`/`makeMemoryStorageLayer`/`MemoryEntry`/`MemoryStorageShape`/`DEFAULT_MEMORY_CAPACITY` re-exports from the `storage/src/index.ts`, `coding-agent/src/core/index.ts`, and `coding-agent/src/index.ts` barrels and the `memory-storage` test. (G-9.3, [deep-open-gaps](scratchpad/reports/deep-open-gaps.md))

### Changed

- `--atif` now emits the Magnitude alpha22-compatible `schema_version:"ATIF-v1.7"` envelope by default. The exported document includes `agent.tool_definitions` (serialized from the active tool set) and a real `final_metrics` block (token and cost totals summed from assistant message usage). Use `--atif-schema legacy|pi|flat` to keep the previous Pi envelope formats.

- Aligned compaction trigger, retention, and failure handling with Magnitude alpha22: trigger now fires at `softCap = min(0.9 * (contextWindow - 8192), 200000)` (previously a fixed `contextWindow - 16384`); recent-context retention is now proportional (`keepRatio * softCap`, default 0.1) instead of a fixed 20000 tokens; and summarization failures degrade to an extractive tail-keep fallback (25%) instead of throwing.

- Aligned the checkpoint system with Magnitude alpha22: registered the `checkpoint_rollback` tool (previously defined but never wired), added `HH:MM:SS` turn-boundary addressing to `checkpoint_changes`/`checkpoint_rollback`, injected a `# Checkpoint System` section into the system prompt when snapshotting is available, exposed `checkpoint_changes` and `checkpoint_rollback` to workers, and enabled auto-snapshot (`experimental.autoSnapshot`) by default.

- Exposed the existing `shell` tool (schema `{command, detach_after?}`) instead of `bash` (`{command, timeout?}`) in all LLM-facing toolkits (worker base, critic base, default active tools, SDK, plan-mode, system prompt) so the model sees the alpha22 wire name and `detach_after` semantics. Internal shell-semantics checks (activity formatter, session orchestrator, mutating-tool guards, permission rules) now accept both `shell` and `bash`.
- Switched git status collection to porcelain v2 (`git status --porcelain=v2`) with a centralized parser (`core/git-state.ts`), and now refresh git branch/status/commits every turn and inject them into the system prompt for all prompt variants (previously captured once at session init and injected only for open-source prompt variants).
...
- Added a third `settings.local.json` merge layer (global → project → local, local highest precedence; read-only from the manager so taste-onboarding state is preserved) and a `permissionRules` setting that sources declarative tool allow/deny rules into sessions (and propagated to spawned workers).
- Added a read-only `git` tool (`status`/`diff`/`log`/`show`/`blame`) as a first-class structured tool, replacing the need to shell out for git inspection (alpha22 parity). Path/ref arguments are validated against absolute paths, `..`, and shell metacharacters; destructive subcommands are not in the allowed set.
- Added `-V` as an alias for `--version` (alpha22 parity: commander auto-registers `-V`).
- Added `--resume [id]` inline session-id form (alpha22 parity): `--resume abc123` opens that specific session by id; bare `--resume` still opens the interactive picker. `main.ts` threads `resumeId` into `SessionManager.open`.
- Added `--debug` activation of `process.env.DEBUG` (alpha22 activates DEBUG alongside `PIKI_DEBUG`).
- Added `serve --debug` flag (alpha22 parity): logs each request method/path to stderr and passes `debug` into `ServeOptions`.
- Added a stateful `serve` control plane (alpha22 parity): `POST /sessions` creates a live `AgentSession` (with orchestrator attached), `POST /sessions/:id/messages`/`interrupt`, `DELETE /sessions/:id`, `GET /sessions/:id/events` + `GET /events` live SSE with `: heartbeat` frames and `Last-Event-ID` replay, `OPTIONS` with CORS `Allow-Methods: GET, POST, DELETE, OPTIONS`, and `/health` → `{status, sessions}`. Replaced runtime-invalid `require("node:fs")` with the top-level `readdirSync` import.

### Fixed

- Restored auto-retry for provider stream errors that explicitly tell the caller to retry the request: OpenAI Responses and Bedrock stream exceptions containing "you can retry your request" / "try your request again" / "please retry your request" now classify as retryable `server_error` in `error-classifier.ts` ([#6019](https://github.com/earendil-works/pi/issues/6019)).
- Closed Magnitude alpha22 shell-classifier parity gaps in `shell-classifier.ts`: attached git config-injection forms (`git -cfoo=bar status`, `--config-env=foo`) are now blocked; docker/podman `--security-opt seccomp=unconfined`/`apparmor=unconfined` (detached and attached) are forbidden; `docker compose down` with `-v`/`--volumes`/`--rmi`/`--remove-orphans` is forbidden (with interleaved flag/value traversal, e.g. `docker compose -f file.yml down -v`); `gcloud auth <non-list>` is forbidden even when value-taking global flags precede it (`gcloud --project foo auth activate-service-account`); and `terraform workspace {new,delete,select}` plus `pulumi stack {rm,init,import}` are forbidden.
- Emitted `observation.results` on alpha22 ATIF compaction steps (carrying the compaction summary under `source_call_id: compaction-<id>`), matching Magnitude alpha22's `compactionPreparedToStep`.
- Closed Magnitude alpha22 shell-classifier parity gaps in `shell-classifier.ts`: attached git config-injection forms (`git -cfoo=bar status`, `--config-env=foo`) are now blocked; docker/podman `--security-opt seccomp=unconfined`/`apparmor=unconfined` (detached and attached) are forbidden; `docker compose down` with `-v`/`--volumes`/`--rmi`/`--remove-orphans` is forbidden (with interleaved flag/value traversal, e.g. `docker compose -f file.yml down -v`); `gcloud auth <non-list>` is forbidden even when value-taking global flags precede it (`gcloud --project foo auth activate-service-account`); and `terraform workspace {new,delete,select}` plus `pulumi stack {rm,init,import}` are forbidden.
- Aligned session sidecar writers with Magnitude alpha22 to avoid the `.meta.json` dual-schema collision: the runtime orchestrator now writes its `SessionRuntimeMeta` to a separate `.runtime-meta.json` sidecar (via `SessionManager.getSessionRuntimeMetaFile()`) instead of clobbering the manager's `.meta.json` preview meta; `.runtime-meta.json` is also cleaned up on session deletion (`SESSION_SIDECAR_SUFFIXES`). ([#A3](https://github.com/earendil-works/pi/issues/A3))
- Aligned new-session guard with Magnitude alpha22 `--headless`: a new session started in non-interactive mode (print/json) with no initial prompt, trailing message, or `--goal` now fails fast with exit 1 and a clear error. ([#A4](https://github.com/earendil-works/pi/issues/A4))
- Aligned RPC-mode resilience with Magnitude alpha22: `runRpcMode` now registers an `unhandledRejection` handler that routes a stray rejection to graceful `shutdown` instead of crashing the persistent RPC host. ([#A5](https://github.com/earendil-works/pi/issues/A5))
- Closed Magnitude alpha22 parity gaps (S11): the spawned shell `M` env now resolves to the session `scratchpadPath` (threaded through `BashToolOptions` → `resolveSpawnContext`/`spawnDetached`/`executeBashWithOperations`, with a `process.env.M` fallback) instead of the outer `process.env.M`; `rm -rf/-fr/-f <system dir>` (`/etc,/usr,/System,/bin,/sbin,/boot,/var,/lib,/dev,/proc,/sys` and descendants) is now `forbidden` at the mag-exact `hasForce` tier (non-force recursive/non-recursive rm of those paths stays allowed); and `web_search` now declares the optional `schema` input and returns `data: undefined` in its result details, matching Magnitude alpha22's contract.
...
- Wired the `query_image` tool to a vision model: added `resolveVisionModel`/`queryVisionModel` to `aux-model.ts` (prefers `featureModels.queryImage`, else the first auth-configured vision-capable model) and passed the callback into the leader's tool build so `query_image` no longer throws "not connected to a vision model".
- Activated `view` for the leader by default by adding it to the core tool auto-activation set alongside `query_image`.
- Registered the `pass` and `escalate` role-control tools for the leader by adding them to the leader tool-registration and activation loops.
- Fixed the `shell` tool's `detach_after` handling. Previously `detach_after` was forwarded to the exec backend as `timeout`, which killed the process on the threshold and surfaced a timeout error instead of detaching it. The tool now runs the command in the foreground and, once `detach_after` elapses, detaches it into a background process via `spawnDetached` (returning the pid/log path) rather than killing it. Remote (non-local) backends fall back to a hard kill-timeout since they cannot detach locally.
- Fixed inverted leader system prompt composition where the model-lineage "family" prompt was the body and `LEADER_PROMPT` was appended as the tail, causing the first instruction to be "You are an expert coding assistant" instead of "You are piki, a highly capable coding agent". The leader identity is now the primary system message and the variant/lineage tuning is appended after it as context, matching Magnitude alpha22.
- Set `catalogLoaded = true` and refined the per-role config profiles (`byRole` context window / max output / capabilities / hard+soft caps) against the live model catalog after session attach (G-W4.4). Previously every role stayed at the hardcoded 200k/16k fallback regardless of the actual model resolving for that role; now each role's caps reflect its resolved model (via `AgentModelResolver`, honoring runtime `roleModels` overrides), matching Magnitude alpha22's post-catalog-load refinement.
- Fixed `pi serve --help` hanging: `runServe` now short-circuits on `-h`/`--help` and prints serve usage instead of starting the server. `serve` is now listed under top-level Commands.
- Fixed a CLI seam gap where `--disable-shell-safeguards` and `--disable-cwd-safeguards` (and any `CreateAgentSessionOptions` field such as `--goal`) set in `buildSessionOptions` were silently dropped at the `createAgentSessionFromServices` call in `main.ts`: added the fields to `CreateAgentSessionFromServicesOptions`, forwarded them through `createAgentSessionFromServices`, and stored `goal` on `AgentSession`. The prior-wave safeguard tests only exercised `createAgentSession` directly and masked the gap.
- Fixed `spawn_worker.yield` semantics to match Magnitude alpha22: `yield: true` is now a cooperative turn-level handoff (`yieldTarget = "workers`) rather than a blocking inline await that returned worker text to the tool caller. `ForkRuntime` records a per-agent yield intent (consumed once at the turn layer) and the orchestrator publishes `worker_finished` with `yield: true`; the worker result is delivered via the `worker-result` custom message, never inline. (The partial blocking implementation was reverted.)
