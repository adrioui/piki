...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.piki/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.piki/extensions/`)
- Project skills:
  - `.piki/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.piki/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.piki/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.piki/agent/auth.json and ~/.piki/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.piki/agent/extensions/`, `.piki/extensions/`, and settings.json extension sources.
...
1. Global: `~/.piki/agent/settings.json`
2. Project: `<cwd>/.piki/settings.json`
