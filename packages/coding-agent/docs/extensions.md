...
> **Placement for /reload:** Put extensions in `~/.piki/agent/extensions/` (global) or `.piki/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.piki/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.piki/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.piki/agent/extensions/*.ts` | Global (all projects) |
| `~/.piki/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.piki/extensions/*.ts` | Project-local |
| `.piki/extensions/*/index.ts` | Project-local (subdirectory) |
...
Fired before pi decides whether to trust a project with dynamic configs (`.piki` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.piki` when constructing project-local config paths. Rebranded distributions can use a different config directory name.
