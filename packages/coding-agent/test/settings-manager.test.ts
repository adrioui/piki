...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates piki starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".piki", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".piki", "settings.json"), JSON.stringify({ theme: "project" }));
		...
		it("should read default project trust from global settings only", () => {
			...
			writeFileSync(join(projectDir, ".piki", "settings.json"), JSON.stringify({ defaultProjectTrust: "never" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .piki folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .piki folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .piki folder that beforeEach created
			rmSync(join(projectDir, ".piki"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .piki folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".piki"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .piki folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .piki folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .piki folder should NOT exist yet
			expect(existsSync(join(projectDir, ".piki"))).toBe(false);
			...
			// Now .piki folder should exist
			expect(existsSync(join(projectDir, ".piki"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".piki", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".piki", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".piki", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
