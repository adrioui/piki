...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.PIKI_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.PIKI_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .piki skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .piki", async () => {
			const extDir = join(tempDir, ".piki", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".piki"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".piki", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".piki", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".piki", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".piki", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".piki", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json piki.extensions in extensions setting", async () => {
			// Create a package with piki.extensions in package.json
			...
			// Should find the extensions declared in package.json piki.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .piki/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-piki", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-piki"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-piki\ndescription: user piki\n---\n");
		...
		it("should use the project .piki dir as baseDir for project .piki skills", async () => {
			const projectBaseDir = join(tempDir, ".piki");
			const skillPath = join(projectBaseDir, "skills", "project-piki", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-piki"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-piki\ndescription: project piki\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".piki", "agent");
		...
		it("should dedupe user skill entries when ~/.piki/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .piki auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".piki\n");

			const skillDir = join(tempDir, ".piki", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with piki manifest", async () => {
		...
		it("should keep piki manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.piki\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".piki", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the piki-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "piki-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .piki settings base", () => {
			...
			const rel = relative(join(tempDir, ".piki"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in piki manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json piki.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".piki", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".piki", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".piki", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".piki", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".piki", "npm", "node_modules", "example");
