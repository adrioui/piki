...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "piki-ext-test-"));
	...
	const extensionCode = `
		export default function(piki) {
			piki.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json piki field", async () => {
	...
	it("keeps package.json piki extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with piki field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without piki field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ piki: { extensions: ["./entry.ts"] } }));
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(piki) {
				piki.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(piki) {
				piki.on("agent_start", async () => {});
				piki.on("tool_call", async (event) => undefined);
				piki.on("agent_end", async () => {});
			}
		`;
