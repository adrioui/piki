...
import { Agent, type AgentEvent, type AgentTool } from "@piki/agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@piki/ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `piki-retry-test-${Date.now()}`);
