...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "piki-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("PIKI_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.PIKI_CODING_AGENT_DIR!, "themes", "foo.json");
