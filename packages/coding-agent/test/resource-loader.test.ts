...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "piki-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".piki", "prompts");
			...
			const projectSkillDir = join(cwd, ".piki", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".piki", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".piki", "themes"), { recursive: true });
	...
	piki.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".piki"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".piki", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".piki", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".piki", "extensions");
...
export default function(piki) {
	...
	piki.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(piki) {
	piki.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".piki", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			writeFileSync(
				join(projectExtDir, "project.ts"),
				`export default function(piki) {
	piki.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	piki.registerCommand("project-only", {
	...
	piki.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.piki", async () => {
			const piDir = join(cwd, ".piki");
	...
	describe("extension conflict detection", () => {
		it("should detect tool conflicts between extensions", async () => {
			...
			writeFileSync(
				...
				`
import type { ExtensionAPI } from "@piki/coding-agent";
import { Type } from "typebox";
export default function(piki: ExtensionAPI) {
  piki.registerTool({
    name: "duplicate-tool",
    description: "First",
