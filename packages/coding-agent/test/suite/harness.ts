...
import type { AgentMessage, AgentTool } from "@piki/agent-core";
import { Agent } from "@piki/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@piki/ai/compat";
import { registerFauxProvider } from "@piki/ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `piki-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
