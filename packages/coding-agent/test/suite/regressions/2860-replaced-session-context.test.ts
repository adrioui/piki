...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `piki-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale piki/ctx", async () => {
		const events: string[] = [];
		...
		const { runtime } = await createRuntimeForTest(
			(piki) => {
				...
				piki.on("session_shutdown", () => {
					events.push(`shutdown:${currentInstance}`);
				});
				piki.registerCommand("repro", {
					description: "repro",
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(piki) => {
				piki.registerCommand("switch-it", {
