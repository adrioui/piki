import type { AgentTool, ThinkingLevel } from "@piki/agent-core";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(piki) => {
					piki.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(piki) => {
					piki.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
	...
	it("keeps activity formatting when extension tool_result handlers observe without modifying", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(piki) => {
					piki.on("tool_result", async () => undefined);
				},
			],
		});
		harnesses.push(harness);
	...
	it("allows extension commands to inspect live system prompt options", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(piki) => {
					piki.registerCommand("inspect-options", {
	...
	it("allows before_agent_start handlers to inject custom messages and modify the system prompt", async () => {
		const harness = await createHarness({
			extensionFactories: [
				(piki) => {
					piki.on("before_agent_start", async (event) => ({
						message: {
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(piki) => {
					piki.on("session_start", async (event) => {
						lifecycleEvents.push(`start:${event.reason}`);
					});
					piki.on("session_shutdown", async (event) => {
