...
import { fauxAssistantMessage, registerFauxProvider } from "@piki/ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `piki-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(piki: ExtensionAPI) => {
						piki.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((piki: ExtensionAPI) => {
			piki.on("message_end", (event) => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((piki: ExtensionAPI) => {
			piki.on("session_before_switch", (event) => {
				events.push(event);
			});
			piki.on("session_shutdown", (event) => {
				events.push(event);
			});
			piki.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `piki-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((piki: ExtensionAPI) => {
			piki.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(
			tmpdir(),
			`piki-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `piki-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `piki-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);
