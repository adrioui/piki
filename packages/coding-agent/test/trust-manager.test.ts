...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".piki", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".piki", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".piki", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".piki"), { recursive: true });
			writeFileSync(join(cwd, ".piki", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".piki"), { recursive: true, force: true });
