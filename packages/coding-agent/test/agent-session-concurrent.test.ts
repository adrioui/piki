...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `piki-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(piki) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = piki;
			},
			(piki) => {
		...
		expect(piki).toBeDefined();

		piki!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
