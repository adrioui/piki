...
import { Agent } from "@piki/agent-core";
import { getModel, type OAuthCredentials, type OAuthProvider } from "@piki/ai/compat";
import { getOAuthApiKey } from "@piki/ai/oauth";
...
// ============================================================================
// OAuth API key resolution from ~/.piki/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".piki", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.piki/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.piki/agent/auth.json
 */
...
/** Path to the real piki agent config directory */
export const PIKI_AGENT_DIR = join(homedir(), ".piki", "agent");

/**
 * Get an AuthStorage instance backed by ~/.piki/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `piki-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
