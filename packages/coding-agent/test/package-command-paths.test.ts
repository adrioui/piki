...
describe("package commands", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `piki-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalPiPackageDir = process.env.PIKI_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".piki"), { recursive: true });
		writeFileSync(join(projectDir, ".piki", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(piki) => {
							piki.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".piki", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".piki"), { recursive: true });
		writeFileSync(join(projectDir, ".piki", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".piki", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".piki", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("piki install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "piki --help" or "piki install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: piki install <source> [-l]");
	...
	it("uses the update check version for forced self updates even when current", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "piki-coding-agent");
...
`,
		...
		process.env.PIKI_PACKAGE_DIR = selfPackageDir;
		...
		try {
			...
			expect(stdout).toContain(`Updated piki from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	...
	it("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "piki-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated piki from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/piki" ? "@newer-scope/piki" : "@new-scope/piki";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		try {
			...
			expect(stderr).toContain("Run `pnpm store prune` and retry `piki update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain(`Updated piki`);
			expect(stderr).toContain("exited with code 23");
			const recordedCalls = JSON.parse(readFileSync(recordPath, "utf-8")) as string[][];
			expect(recordedCalls).toEqual([
				expect.arrayContaining(["uninstall", "-g", PACKAGE_NAME]),
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:piki-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "piki-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:piki-formatter?");
			expect(stdout).not.toContain("Updated piki-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:piki-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
