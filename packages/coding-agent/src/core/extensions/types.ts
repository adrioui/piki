...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@piki/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@piki/coding-agent";
	 *
...
export interface ToolDefinition<TParams extends TSchema = TSchema, TDetails = unknown, TState = any> {
	...
	/** When true, the tool is callable but hidden from the system prompt's tool documentation. */
	hidden?: boolean;

	/**
	 * Optional streaming input handler. Invoked before/while tool arguments are
	 * being assembled (during partial argument streaming) to validate or react
	 * to incremental input. Mirrors the harness tool `stream` capability so the
	 * public tool registry exposes the same surface as harness-defined tools.
	 */
	stream?: {
	...
	/**
	 * Optional Effect Schema describing events the tool emits during execution
	 * (streamed progress payloads). Present for structural inspection parity
	 * with harness-defined tools.
	 */
	emissionSchema?: unknown;

	/**
	 * Optional Effect Schema describing the tool's error shape. Present for
	 * structural inspection parity with harness-defined tools.
	 */
	errorSchema?: unknown;
